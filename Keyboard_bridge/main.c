...
static void send_uart_key(uint8_t ascii)
{
    uint8_t hid = ascii_to_hid(ascii);
    ...
    if (shift)
        hid_keyboard_state.ctrl_keys_state |= SHIFT_KEY_CODE;

    hid_buttons_press(&hid, 1);
    k_msleep(10);
    hid_buttons_release(&hid, 1);
...
K_THREAD_DEFINE(uart_thread_id,
                2048,
                uart_thread,
                NULL, NULL, NULL,
                7, 0, 0);
...
printk("UART console initialized\n");
