...
\begin{itemize}
    ...
    \item \textbf{Smartphone:} Android device with nRF Connect for Mobile and Serial Bluetooth Terminal App.
...
\begin{tikzpicture}[node distance=2cm, auto, thick]
    ...
    % Arrows
    \draw[<->] (pc) -- node[above, align=center, font=\scriptsize] {Micro USB-CDC\\(Wired)} (nrf);
...
\begin{tikzpicture}[node distance=2.5cm, auto, thick]
    ...
    % Connection
    \draw[<->, line width=1.2pt] (pc) -- node[above, font=\scriptsize] {Micro-USB Cable} node[below, font=\scriptsize] {(Power + UART)} (nrf);
\end{tikzpicture}
\vspace{0.2cm}
...
\begin{enumerate}
    ...
    \item \textbf{Result:} The content of the file will stream immediately into the open \texttt{minicom} session on your Linux terminal. This can be saved as a file in the PC.
\end{enumerate}
